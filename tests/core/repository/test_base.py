...
class TestBaseRepository:
    ...
    async def repository(self, db_session: AsyncSession) -> BaseRepository:
        return BaseRepository(model=User, db_session=db_session)

    @pytest.mark.asyncio
    async def test_create(self, repository: BaseRepository):
        user = await repository.create(self._user_data_generator())
        await repository.session.commit()
        assert user.id is not None

    @pytest.mark.asyncio
    async def test_get_all(self, repository: BaseRepository):
